...
trainx <- read.delim(file.path("train", "X_train.txt"), sep="", header=FALSE)
testx <- read.delim(file.path("test", "X_test.txt"), sep="", header=FALSE)
allx <- rbind(trainx, testx)

#Read and combine the training and testing Y data sets
trainy <- read.delim(file.path("train", "y_train.txt"), sep="", header=FALSE)
testy <- read.delim(file.path("test", "y_test.txt"), sep="", header=FALSE)
ally <- rbind(trainy, testy)

#Read and combine the training and testing subject lists
subjecttrain <- read.delim(file.path("train", "subject_train.txt"), sep="", header=FALSE)
subjecttest <- read.delim(file.path("test", "subject_test.txt"), sep="", header=FALSE)
...
write.table(final2, file = "tidy_1.txt", row.names = FALSE)
