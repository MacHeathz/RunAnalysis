...
# Load the given file using read_fwf or fread, and create a dplyr table from it using
# as.tbl().
...
#
load_fwf <- function(filename, ...) {
  read_fwf(filename, ...)
}
load_fread <- function(filename, ...) {
  as.tbl(fread(filename, ...))
}
...
#
load_data_subset <-
  function(base_dir, data_subset, activity_labels) {
    print(paste0("Loading ", data_subset, "ing data."))
    dir <- file.path(base_dir, data_subset)
    
    # Read data plus subjects and activities from files
    theList <- rep_len(c(16), 561)
    data <- load_fwf(file.path(dir, paste0("X_", data_subset, ".txt")), fwf_widths(theList))
    subjects <-
      load_fread(file.path(dir, paste0("subject_", data_subset, ".txt")))
    activities <-
      load_fread(file.path(dir, paste0("y_", data_subset, ".txt")))
    ...
    named_activities <-
      full_join(activities, activity_labels, by = "V1")[2]
...
load_data <- function(dir) {
  ...
  activity_labels <-
    load_fread(file.path(dir, "activity_labels.txt"))
  ...
  train_data <-
    load_data_subset(dir, "train", activity_labels) %>% setNames(col_names)
  test_data <-
    load_data_subset(dir, "test", activity_labels) %>% setNames(col_names)
