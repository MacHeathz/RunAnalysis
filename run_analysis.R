...
run_analysis <- function() {
  ...
  load_file <- function(filename) {
    # A trick shown in R Programmming course: setting
    # colClasses like this speeds up read.table a lot!
    initial <- read_table(filename, nrows = 100)
    found_classes <- sapply(initial, class)
    as.tbl(read_table(filename), colClasses = found_classes)
  }
  ...
  load_data_subset <-
    function(base_dir, data_subset, activity_labels) {
      ...
      data <-
        load_file(file.path(dir, paste("X_", data_subset, ".txt", sep = "")))
      subjects <-
        load_file(file.path(dir, paste("subject_", data_subset, ".txt", sep = "")))
      activities <-
        load_file(file.path(dir, paste("y_", data_subset, ".txt", sep = "")))
  ...
  load_data <- function(dir) {
    ...
    activity_labels <-
      load_file(file.path(dir, "activity_labels.txt"))
